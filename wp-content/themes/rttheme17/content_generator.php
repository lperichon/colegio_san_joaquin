					...
					if($template->content_type=="code_box"){ 			 
						...
						if($layout!="one"){
							...
							if($transparent)
								...
								echo '<div id="code_box_'.$template->group_id.'" style="'.$inlineCSS2.'" class="template_builder box-shadow '.$layout.' '.$firstBox.'  '.$lastBox.'">';							
						}
								echo ($template->heading) ? '<h3>'.wpml_t( THEMESLUG , 'Title for Code Box '.$selectedTemplate->templateID, stripslashes($template->heading) ).'</h3><div class="space margin-b10"></div>' :"";
